...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  if model_architecture == 'single_fc':
    ...
    return create_tiny_embedding_conv_model(fingerprint_input, model_settings,
                                            is_training)
  elif model_architecture == 'ada':
    return create_ada_model(fingerprint_input, model_settings, is_training)
...
def create_ada_model(fingerprint_input, model_settings, is_training):
  if is_training:
    dropout_rate = tf.compat.v1.placeholder(tf.float32, name='dropout_rate')
  fingerprint_size = model_settings['fingerprint_size']
  label_count = model_settings['label_count']
